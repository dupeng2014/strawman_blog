...
# 观察向量化的数据
print(''.join([input_dict_reverse[np.argmax(i)] for i in encoder_input[1000] if max(i) !=0]))
print(''.join([target_dict_reverse[np.argmax(i)] for i in decoder_output[1000] if max(i) !=0]))
print(''.join([target_dict_reverse[np.argmax(i)] for i in decoder_input[1000] if max(i) !=0]))
